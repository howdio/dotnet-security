...
namespace Howdio.Security.Utils
{
    ...
    {
        private static readonly Packer SwapConverter = new SwapConverterImplementation();
        private static readonly Packer CopyConverter = new CopyConverterImplementation();
        ...
        {
            get { return BitConverter.IsLittleEndian ? CopyConverter : SwapConverter; }
        }

        static public Packer BigEndian
        {
            get { return BitConverter.IsLittleEndian ? SwapConverter : CopyConverter; }
        }

        static public Packer Native
        {
            get { return CopyConverter; }
        }
        ...
        static bool PackOne(PackContext b, object oarg)
        {
            ...
            {
                case '^':
                    b.converter = BigEndian;
                    return false;
                case '_':
                    b.converter = LittleEndian;
                    return false;
                case '%':
                    b.converter = Native;
                    return false;
                ...
                // Type Conversions
                case 'i':
                    b.Add(b.converter.GetBytes(Convert.ToInt32(oarg)));
                    break;

                case 'I':
                    b.Add(b.converter.GetBytes(Convert.ToUInt32(oarg)));
                    break;
                case 's':
                    b.Add(b.converter.GetBytes(Convert.ToInt16(oarg)));
                    break;
                case 'S':
                    b.Add(b.converter.GetBytes(Convert.ToUInt16(oarg)));
                    break;
                case 'l':
                    b.Add(b.converter.GetBytes(Convert.ToInt64(oarg)));
                    break;
                case 'L':
                    b.Add(b.converter.GetBytes(Convert.ToUInt64(oarg)));
                    break;
                case 'b':
                ...
                case 'C':
                    ...
                    break;
                case 'X':
                    b.Add(b.converter.GetBytes(((IBinarySerializable)oarg).ToByteArray()));
                    break;
                case 'A':
                    b.Add(b.converter.GetBytes((byte[])oarg));
                    break;
        ...
        {
            Packer converter = CopyConverter;
            ...
            for (var i = 0; i < description.Length && idx < buffer.Length;)
            {
                ...
                switch (description[i])
                {
                    ...
                    case 'i':
                        ...
                        {
                            result.Add(converter.ToInt32(buffer, idx));
                            idx += 4;
                        }
                        break;
                    case 'I':
                        if (Prepare(buffer, ref idx, 4, ref align))
                        {
                            result.Add(converter.ToUInt32(buffer, idx));
                            idx += 4;
                        }
                        break;
                    case 's':
                        ...
                        {
                            result.Add(converter.ToInt16(buffer, idx));
                            idx += 2;
                        }
                        break;
                    case 'S':
                        if (Prepare(buffer, ref idx, 2, ref align))
                        {
                            result.Add(converter.ToUInt16(buffer, idx));
                            idx += 2;
                        }
                        break;
                    case 'l':
                        ...
                        {
                            result.Add(converter.ToInt64(buffer, idx));
                            idx += 8;
                        }
                        break;
                    case 'L':
                        if (Prepare(buffer, ref idx, 8, ref align))
                        {
                            result.Add(converter.ToUInt64(buffer, idx));
                            idx += 8;
                        }
                        break;
                    case 'b':
                    ...
                    case 'C':
                        if (Prepare(buffer, ref idx, 1, ref align))
                        {
                            char c = description[i] == 'c' ? ((char)((sbyte)buffer[idx])) : ((char)(buffer[idx]));
                            result.Add(c);
                            idx++;
                        }
                        break;
                ...
                i = repeat > 0 && --repeat > 0 ? save : i + 1;
            }
            return result;
        }

        class CopyConverterImplementation : Packer
        {
        ...
        class SwapConverterImplementation : Packer
        {
