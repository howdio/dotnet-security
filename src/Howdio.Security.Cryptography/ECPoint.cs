...
namespace Howdio.Security.Cryptography
{
    public class ECPoint : IComparable<ECPoint>, IEquatable<ECPoint>
    {
        ...
        public static ECPoint Decode(byte[] encoded, ECCurve curve)
        {
            ECPoint p = null;
        ...
        private static ECPoint Decompress(int yTilde, BigInteger X1, ECCurve curve)
        {
        ...
        public static ECPoint Deserialize(BinaryReader reader, ECCurve curve)
        {
            int expectedLength = (curve.Q.GetBitLength() + 7) / 8;
            ...
            switch (buffer[0])
            {
                ...
                case 0x03:
                    ...
                    return Decode(buffer.Take(1 + expectedLength).ToArray(), curve);
                case 0x04:
                case 0x06:
                case 0x07:
                    reader.Read(buffer, 1, expectedLength * 2);
                    return Decode(buffer, curve);
                default:
                    throw new FormatException("Invalid point encoding " + buffer[0]);
            }
        }

        public byte[] Encode(bool commpressed)
        {
        ...
        public static ECPoint FromBytes(byte[] pubkey, ECCurve curve)
        {
            switch (pubkey.Length)
            {
                case 33:
                case 65:
                    return Decode(pubkey, curve);
                case 64:
                case 72:
                    return Decode(new byte[] { 0x04 }.Concat(pubkey.Skip(pubkey.Length - 64)).ToArray(), curve);
                case 96:
                case 104:
                    return Decode(new byte[] { 0x04 }.Concat(pubkey.Skip(pubkey.Length - 96).Take(64)).ToArray(), curve);
                default:
        ...
        {
            return Decode(value.ToByteArray(), curve);
        }

        public override string ToString()
        {
            return Encode(true).ToHexString();
        }
